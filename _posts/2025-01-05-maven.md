...
* 首先，Maven 是一个优秀的项目构建工具。
  
  使用 maven，可以很方便的对项目进行分模块构建，这样在开发和测试打包部署时，效率会提高很多。
* 其次，Maven 可以进行依赖的管理。
...
这个也好解决，在 <mirrors> 中增加阿里巴巴的 Maven 私服 https://developer.aliyun.com/mvn/guide。
## 6.什么是Maven的坐标
...
test-compile - 将需测试源代码编译到路径。

一般来说，是编译/src/test/java目录下的java文件至目标输出的测试classpath目录中。
