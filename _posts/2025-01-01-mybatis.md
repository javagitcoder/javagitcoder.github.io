...
当你执行查询主对象的SQL时（如 selectUserById），MyBatis 返回的 User 对象中，orders 属性并不是真正的 List&lt;Order&gt;，而是一个由 MyBatis 生成的、实现了 List&lt;Order&gt; 接口的代理对象。

这个代理对象内部保存了能够单独查询订单数据的SQL语句和参数。

当你第一次调用 user.getOrders() 方法时，这个代理对象会被触发，它去执行之前保存的SQL，查询出真正的订单数据，然后返回给你。
